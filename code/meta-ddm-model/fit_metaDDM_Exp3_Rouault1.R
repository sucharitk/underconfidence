...
####################################################################################
###############################################################################
####### Fit model to Dataset 3 (Rouault et al, Exp 1) ##############
###############################################################################
...
df <- read.csv('rouault_2018_exp1.csv', header = TRUE, sep = ',') %>% drop_na()
...
if (fit.vratio){
  ...
  additional <- c(1, .005, 1, 1, 4000, nconflevels)
...
k2 <- list(recover, bestfnval, AD.score, gender, age, exp.name, subjs)
names(k2) <- c('recover', 'bestfnval', 'AD', 'gender', 'age', 'expname', 'subj')
