...
####################################################################################
###############################################################################
####### Fit model to Dataset 1 (Katyal et al, Exp 1) ##############
###############################################################################
...
## read data file
df = read.csv('data/Katyal_et_al_2023/mbsExp1_noperfexcl.csv', header = TRUE, sep = ',')
...
df <- df %>%mutate_at(c("group", 'gender'), as.factor) %>%
...
if (fit.vratio){
  ...
  additional <- c(1, .005, 1, 1, 3000)
  lower <- c(0, .5,  0.05, .2, -7, .02, -5)
  upper <- c(3,  4,  .2,   2,  7,   5,  5)
}
...
task2fit <- 2 # 1-perception, 2-memory: run for each value
...
for (ns in next.subj:nsubj){
  ...
  if (nrow(tempDat) > 1 && mean.accu>.4 && mean.accu<.95){
...
k2 <- list(recover, bestfnval, AD.score, gender, age, subjs, exp.name, 
           taskNames[task2fit])
names(k2) <- c('recover', 'bestfnval', 'AD', 'gender', 'age', 'subj', 'expname', 
               'task')
